...
export default {
  ...
  mutations: {
    ...
    updateItem(state, { id, title, desc, type, status, date, deadline, color }) {
      const items = state[type].concat()
      const index = items.findIndex(el => el.id === id)
      let el = items[index]
      items[index] = { ...el, title, desc, type, status, date, deadline, color }
      state[type] = items
    },
  ...
  actions: {
    ...
    updateItem({ commit }, { id, title, desc, type, status, date, deadline, color }) {
      const ref = db.collection("users").doc(auth.currentUser.uid)
      const el = ref.collection(type).doc(id).get().then(doc => doc.data())
      return ref.collection(type).doc(id).update({ ...el, title, type, status, date, deadline, color })
        .then(function () {
          console.log("updateItem, Document successfully updated!");
          commit('updateItem', { id, title, desc, type, status, date, deadline, color })
        })
        .catch(function (error) {
